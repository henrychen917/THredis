...
class ScopeTest(unittest.TestCase):
    ...
    def test_r6_adapter_moves_scope_to_io_and_keeps_fifo_control(self):
        ex = scope.instrument((scope.ROOT / "src/core/ex_loop.h").read_text())
        io = scope.instrument_io((scope.ROOT / "src/core/io_loop.h").read_text())
        ex, io, probe = scope.instrument_r6(ex, io, scope.PROBE)
        self.assertNotIn("reorder_scope::Batch scope_batch", ex)
        self.assertEqual(io.count("reorder_scope::admit(srv_->cfg(), batch, count);"), 1)
        self.assertIn("reorder_enabled_ = true;", io)
        self.assertIn("? ex_schedule_batch(batch, count) : ReorderResult{};", io)
        self.assertIn("admission_samples.erase(it);", probe)
        self.assertIn("it->second.window == recorder.window", probe)
