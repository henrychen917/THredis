...
# R6 adapter to the shared R1/R2 instrument. Only generated diagnostic sources change.
# The map joins immutable pre-publication metadata to the first execution attempt; no
# Op/Client/argv is read at timer teardown. Reorder=0 in a diagnostic R6 copy admits
# the same cohorts in FIFO order, so S3 can observe the point before enabling policy.
# This perturbation (and the timing/map overhead) is why these rates are unscored.
R6_ADMISSION_PROBE = r"""
using AdmissionId = std::tuple<uintptr_t, uint64_t, int32_t, uintptr_t>;
struct AdmissionSample { uint64_t window; std::string key; };
inline std::mutex admission_mutex;
inline std::map<AdmissionId, AdmissionSample> admission_samples;
inline AdmissionId admission_id(const Task& task) {
    return {reinterpret_cast<uintptr_t>(task.client), task.op_id, task.shard,
            reinterpret_cast<uintptr_t>(task.scatter)};
}
inline void admit(const Config& cfg, const Task* tasks, uint32_t n) {
    Batch batch(tasks, n);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (batch.sampled) {
            ...
            const Op* op = e.task.client ? &e.task.client->rob().at(e.task.op_id) : nullptr;
            key = metadata(cfg, op, e.eligible ? "eligible" : "barrier",
                e.eligible ? e.length : -1, e.rank, i, n, e.later_short);
        }
        // An unsampled cohort still needs a marker: otherwise EX could independently sample
        // its tasks and falsely label them outside admission. This map is diagnostic-only.
        admission_samples.insert_or_assign(admission_id(tasks[i]), AdmissionSample{
            recorder.window, std::move(key)});
    }
}
"""


def instrument_r6(ex, io, probe):
    for hook in ("        reorder_scope::Batch scope_batch(batch, n);\n",
                 "                reorder_scope::Batch scope_batch(batch, held);\n"):
        ex = replace_once(ex, hook, "")
    io = replace_once(io, "        reorder_enabled_ = srv_->cfg().reorder != 0;",
                      "        reorder_enabled_ = true; // Unscored S3 FIFO cohorts on reorder=0.")
    anchor = "                    const ReorderResult result = ex_schedule_batch(batch, count);"
    io = replace_once(io, anchor, """                    reorder_scope::admit(srv_->cfg(), batch, count);
                    const ReorderResult result = srv_->cfg().reorder
                        ? ex_schedule_batch(batch, count) : ReorderResult{};""")
    anchor = "                    srv_->mode_schedule_stats(self_->id()).note_reorder(count, result);"
    io = replace_once(io, anchor, "                    if (srv_->mode_schedule_stats())\n" + anchor)
    probe = replace_once(probe, "#include <map>", "#include <map>\n#include <mutex>\n#include <tuple>")
    probe = replace_once(probe, "struct Execution : Timer {", R6_ADMISSION_PROBE + "\nstruct Execution : Timer {")
    anchor = """        if (current_batch) {
            if (!current_batch->sampled) return;
        } else {
            recorder.checkpoint();
            if (!recorder.sample()) return;
        }"""
    probe = replace_once(probe, anchor, r"""        recorder.checkpoint();
        if (!recorder.window) return;
        {
            std::lock_guard lock(admission_mutex);
            auto it = admission_samples.find(admission_id(task));
            if (it != admission_samples.end()) {
                std::string key;
                if (it->second.window == recorder.window) key = std::move(it->second.key);
                admission_samples.erase(it);
                if (!key.empty()) {
                    // A task captured on IO but first attempted from a retry queue belongs
                    // to that execution population. Keep its IO rank/position for diagnosis.
                    if (std::string(outside_path) != "outside") {
                        size_t begin = 0;
                        for (unsigned field = 0; field < 7; field++) begin = key.find('\t', begin) + 1;
                        key.replace(begin, key.find('\t', begin) - begin, outside_path);
                    }
                    start(std::move(key));
                }
                return;
...
def prepare(path, revision=None):
    ...
    probe = PROBE
    admission = "void finish_admission() {" in original_io
    if admission:
        patched, patched_io, probe = instrument_r6(patched, patched_io, probe)
    ...
    manifest = {"scope_point": "io-admission" if admission else "executor-gather",
                "source_ref": revision, "source_sha256": digest.hexdigest(),
