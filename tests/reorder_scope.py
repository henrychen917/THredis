...
key/value contents are recorded.
...
def instrument(source, measured=False):
    ...
    for anchor, hook in (
        ...
        if measured:
            # R4 shares the R1 probe and populations. Only the predictor adapter differs;
            # the disposable instrument may read this physical worker's private cost table.
            hook = ("        reorder_scope::current_costs = reorder_enabled_ ? "
                    "srv_->mode_schedule_stats(self_->id()).reorder_costs.get() : "
                    "&reorder_scope::unknown_costs;\n" + hook)
...
def probe_source(measured):
    if not measured:
        return PROBE
    probe = replace_once(PROBE, "namespace tomo::reorder_scope {",
        "namespace tomo::reorder_scope {\n"
        "inline const ExReorderCosts unknown_costs;\n"
        "inline thread_local const ExReorderCosts* current_costs = &unknown_costs;")
    probe = replace_once(probe, "ex_sched_candidate(e.task, e.length)",
                         "ex_sched_candidate(e.task, e.length, *current_costs)")
    probe = replace_once(probe, "    uint8_t length = 0;", "    uint8_t length = 0;\n    int predicted = -1;")
    probe = replace_once(probe,
        "            e.eligible = ex_sched_candidate(e.task, e.length, *current_costs);",
        "            e.eligible = ex_sched_candidate(e.task, e.length, *current_costs);\n"
        "            if (e.eligible && current_costs != &unknown_costs)\n"
        "                e.predicted = current_costs->class_for(e.task.client->rob().at(e.task.op_id).spec->id);")
    probe = replace_once(probe, "int effective, int rank, int position, int batch, bool later)",
                         "int effective, int rank, int position, int batch, bool later, int predicted = -1)")
    probe = replace_once(probe,
        "found ? current_batch->count : 0, found && found->later_short));",
        "found ? current_batch->count : 0, found && found->later_short, found ? found->predicted : -1));")
    return replace_once(probe,
        "op && op->spec ? static_cast<int>(op->spec->length_class) : -1,",
        "predicted >= 0 ? predicted : current_costs != &unknown_costs && op && op->spec && "
        "op->spec->id < ExReorderCosts::kCommands\n"
        "            ? static_cast<int>(current_costs->class_for(op->spec->id)) : -1,")
...
def instrument_io(source):
    ...
    source = replace_once(source, anchor, anchor + '''
                ...
                if (reorder_scope::recorder.window)
...
def prepare(path, revision=None):
    ...
    measured = "exec_batch_reordered" in original
    patched = instrument(original, measured)
    ...
    (path / "src/core/reorder_scope_probe.h").write_text(
        probe_source(measured).replace("@DIRECTORY@", json.dumps(str(path))))
